...
try {
  ...
  if (padSizes && (padSizes.washable !== dimsConfig.washablePad.width || padSizes.disposable !== dimsConfig.disposablePad.width)) {
  ...
  if (ok) {
    ...
    // Additional checks: grid divisibility
    const grid = 0.25;
    const checkDiv = (val, name) => {
      const eps = 1e-9;
      const q = Math.round(val / grid);
      if (Math.abs(q * grid - val) > eps) {
        fail(`${name} (${val}) is not an integer multiple of grid ${grid}`);
        ok = false;
      }
    };
    // check all widths/depths
    for (const k of Object.keys(dimsConfig)) {
      const d = dimsConfig[k];
      checkDiv(d.width, `${k}.width`);
      checkDiv(d.depth, `${k}.depth`);
    }
    checkDiv(penSize, 'PEN_SIZE');
    if (ok) {
      console.log('Grid alignment OK: all dimensions are integer multiples of 0.25"');
      process.exitCode = 0;
    } else {
      console.error('\nAUDIT FAILED: grid alignment issues found.');
      process.exitCode = 2;
    }
  } else {
